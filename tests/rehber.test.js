...
run('duplicates = [{phone:"905321111111",people:[newContact({id:"1",firstName:"A"}),newContact({id:"2",firstName:"B"})]},{phone:"905322222222",people:[newContact({id:"3",firstName:"C"}),newContact({id:"4",firstName:"D"}),newContact({id:"5",firstName:"E"}),newContact({id:"6",firstName:"F"})]}]; renderDuplicates()');
const cards = element('duplicateList').children;
assert.equal(cards.length, 2);
assert.equal(cards[0].children[1].children.length, 2);
assert.equal(cards[1].children[1].children.length, 4);
assert.equal(cards[0].children[2].className, 'duplicate-actions');
assert.equal(cards[1].children[2].className, 'duplicate-actions');
